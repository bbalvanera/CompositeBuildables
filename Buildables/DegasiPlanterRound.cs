...
public static class DegasiPlanterRound
{
    ...
    public static IEnumerator ModifyPrefabAsync(GameObject obj)
    {
        ...
        var planterModel = obj.transform.Find("model").gameObject; // Holds model called "Base_interior_Planter_Pot_02"

        // Stretch the Pot model
        planterModel.transform.Find("Base_interior_Planter_Pot_02").localScale = new Vector3(3, 3, 1);
        ...
        var box = obj.transform.Find("Collider").GetComponent<BoxCollider>(); // This one is used for constructed objects
        var size = box.size;
        size.x *= 3;
        size.z *= 3;
        box.size = size;

        var cb = obj.GetComponent<ConstructableBounds>(); // This one is used for the pre-construction hologram
        size = cb.bounds.size;
        size.x *= 3;
        size.z *= 3;
        cb.bounds.size = size;
        ...
        Object.Destroy(obj.transform.Find("StoragetRoot").gameObject);
        Object.Destroy(obj.transform.Find("grownPlant").gameObject);
    ...
    {
        // create prefab:
        var planterPrefab = new CustomPrefab(Info);

        // Initializes planterPrefab.Info to the argument Info
        var planterClone = new CloneTemplate(Info, "5c8cb04b-9f30-49e7-8687-0cbb338fc7fa"); // model is stored in object called "model"
